...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden max-w-4xl">
    ...
    <div class="px-6 pb-6 mt-[-1rem]">
        <form action="{{ route('superadmin.tenant.destroy', $tenant->id) }}" method="POST" class="inline-block" onsubmit="confirmFormSubmit(event, 'Apakah Anda yakin ingin menghapus tenant ini?')">
            @csrf
            @method('DELETE')

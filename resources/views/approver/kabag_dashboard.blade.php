...
@section('content')
    <!-- Header Page -->
    <div class="mb-8">
        <h1 class="text-[26px] font-bold text-gray-900 tracking-tight mb-1">Antrean Persetujuan (Kabag)</h1>
        <p class="text-[15px] text-gray-500 font-medium">Daftar usulan pencairan dana yang telah disetujui Kaur dan menunggu persetujuan akhir (Kabag).</p>
    </div>
    ...
    <div class="bg-white rounded-[20px] shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        ...
        <!-- Filter & Search Bar -->
        <form method="GET" action="{{ route('kabag.dashboard') }}" class="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <!-- Search -->
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    @empty
                    <x-empty-state icon="ph ph-file-text" title="Belum ada antrean" message="Tidak ada antrean persetujuan saat ini." :colspan="8" />
                    @endforelse
                </tbody>
            </table>
        </div>

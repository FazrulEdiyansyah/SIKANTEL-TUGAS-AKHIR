...
@section('title', 'Edit Data Kantin - SIKANTEL')
...
@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            <div class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                ...
                <span class="text-gray-900 font-semibold">Edit Kantin</span>
            </div>
            <h1 class="text-[26px] font-bold text-gray-900 tracking-tight">Edit Kantin: {{ $kantin->nama_kantin }}</h1>
        </div>
    </div>

    <div class="bg-white rounded-[20px] shadow-sm border border-gray-100 overflow-hidden max-w-3xl">
        <div class="p-8">
            <form action="{{ route('pengelola.kantin.update', $kantin->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="nama_kantin" id="nama_kantin" required placeholder="Contoh: Kantin GKU" value="{{ old('nama_kantin', $kantin->nama_kantin) }}" 
                ...
                <div>
                    ...
                    <input type="text" name="lokasi" id="lokasi" required placeholder="Contoh: Gedung Kuliah Umum" value="{{ old('lokasi', $kantin->lokasi) }}"
                ...
                <!-- Foto Thumbnail -->
                <div>
                    <label for="foto" class="block text-sm font-semibold text-gray-700 mb-2">Foto / Thumbnail Kantin</label>
                    
                    <!-- Upload Area -->
                    <div id="upload-area" class="{{ $kantin->foto ? 'hidden' : '' }} mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-200 border-dashed rounded-xl hover:border-telkom-red/50 hover:bg-red-50/50 transition-colors cursor-pointer relative" onclick="document.getElementById('foto').click()">
                    ...
                    <div id="preview-area" class="{{ $kantin->foto ? '' : 'hidden' }} mt-1">
                        ...
                        <div class="flex items-center justify-center space-x-3">
                            ...
                            <button type="button" onclick="removeImage()" class="px-4 py-2 bg-red-50 border border-red-100 text-red-600 rounded-xl text-sm font-bold hover:bg-red-100 transition-colors w-full flex justify-center items-center"><i class="ph ph-trash mr-2"></i> Hapus Foto (Reset)</button>
                        </div>
                    </div>
                ...
                <div class="pt-6 mt-4 border-t border-gray-100 flex items-center justify-end space-x-3">
                    <a href="{{ route('pengelola.kantin.show', $kantin->id) }}" class="px-6 py-2.5 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-100 transition-colors">Batal</a>
    ...
    <script>
        ...
        function removeImage() {
            ...
            // If it already had a photo from DB, we might want to just show the old one 
            // or let it clear and if they submit without file, we keep old.
            // But visually we clear it here.
            imagePreview.src = '#';

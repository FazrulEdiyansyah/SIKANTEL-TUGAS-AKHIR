...
<head>
    <meta charset="UTF-8">
    <title>Laporan Pencairan Dana - {{ $tenant->nama_tenant }}</title>
    <style>
...
<body>
    ...
    <table class="info-table">
        ...
        <tr>
            ...
            <td>: {{ $tenant->kantin->nama_kantin ?? '-' }}</td>
        </tr>
        <tr>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">Rp {{ number_format($danaTelu, 0, ',', '.') }}</td>
            </tr>
            <tr>
                <td>3</td>
                <td><strong>Dana Cair (Tenant 70%)</strong></td>
                <td class="text-right"><strong>Rp {{ number_format($danaTenant, 0, ',', '.') }}</strong></td>
            </tr>
        </tbody>
    </table>

    <p style="font-size: 12px; color: #666; margin-top: -15px; margin-bottom: 40px;">
        * Laporan ini merupakan preview dari sistem SIKANTEL dan belum disetujui oleh Kepala Urusan maupun Kepala Bagian kecuali jika diajukan.
    </p>
    ...
    <div class="footer">
        <div class="signature-box">
            ...
            <div class="signature-name">{{ auth()->user()->name ?? 'Pengelola' }}</div>
        </div>
    </div>
